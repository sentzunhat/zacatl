# Reference Documentation

In-depth technical documentation for Zacatl.

## Sections

- **[API Reference](./api/README.md)** - Complete API documentation
- **[Architecture](./architecture/README.md)** - System design and patterns
- **[ORM & Databases](./orm/README.md)** - Database integration guide
- **[Path Aliases](./path-aliases.md)** - TypeScript path aliases (v0.1.0+)
- **[Third-Party Dependencies](./third-party.md)** - Re-exported packages

## Quick Links

### Developer Experience

- [Path Aliases](./path-aliases.md) - `@zacatl/*` imports for cleaner code
- [Third-Party Dependencies](./third-party.md) - tsyringe, zod, ORM packages

### API Reference

- [Configuration](./api/configuration.md) - Configuration options
- [Errors](./api/errors.md) - Error handling
- [i18n](./api/i18n.md) - Internationalization API
- [Logging](./api/logging.md) - Logging API
- [Repository](./api/repository.md) - Repository pattern
- [Service](./api/service.md) - Service configuration

### Architecture

- [Framework Overview](./architecture/framework-overview.md) - How Zacatl works
- [Multi-Context Design](./architecture/multi-context-design.md) - CLI, Desktop, Server support (v0.1.0+)
- [Architecture Decisions](./architecture/decisions/) - Design decision records

### ORM & Databases

- [ORM Overview](./orm/overview.md) - Supported databases
- [ORM Architecture](./orm/architecture.md) - How adapters work
- [Import Strategies](./orm/orm-import-strategies.md) - Different import patterns
- [Lazy Loading](./orm/orm-lazy-loading.md) - Load related data efficiently
- [Multi-ORM Setup](./orm/multi-orm-setup.md) - Use multiple databases

## Learning Path

**New to Zacatl?** Start with [Tutorials](../tutorials/README.md)  
**Building something?** Check [Guides](../guides/README.md)  
**Need details?** You're in the right place!
