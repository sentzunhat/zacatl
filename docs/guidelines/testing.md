...
│   │   └── infrastructure/
│   │       └── orm/
│   │           ├── mongoose/adapter-loader.ts
│   │           └── sequelize/adapter-loader.ts
│   └── platforms/
│       ├── cli/cli.ts
...
├── service/
│   ├── service.test.ts
│   ├── layers/infrastructure/orm/
│   │   ├── adapter-loader.lazy.test.ts
│   │   └── sequelize/adapter.test.ts
│   └── platforms/
│       ├── cli/cli.test.ts
│       ├── context/request-context.test.ts
│       ├── desktop/desktop.test.ts
│       └── server/
│           ├── server.test.ts
│           ├── api/express-adapter.test.ts
│           └── database/sqlite-adapter.test.ts
├── error/
│   └── custom.test.ts
└── utils/
    └── measure-execution-time.test.ts
```
...
### Mocking with `vi.mock` and `vi.hoisted`

Vitest hoists `vi.mock(...)` calls to the top of the file before any `import` statements. If your mock factory references a local `const` defined in the same scope, you must wrap it in `vi.hoisted()` to avoid a `ReferenceError`:

```typescript
// ✅ Correct — vi.hoisted ensures the fn is available when vi.mock runs
...
> This applies any time a `vi.mock` factory captures an outer-scope variable. See [Vitest docs on `vi.hoisted`](https://vitest.dev/api/vi.html#vi-hoisted).
...
> Current baseline: ~71% statements / lines. The target floor is 70% statements and 75% functions.
...
export default defineConfig({
  test: {
    coverage: {
      provider: 'istanbul', // NOT v8 — istanbul is configured in vite.config.mjs
      reporter: ['text', 'lcov'],
      ...
      exclude: [
        ...
        'src/utils/optionals.ts', // Optional utilities
      ],
      lines: 70,
      functions: 75,
      branches: 60,
...
```typescript
// Mark lines as intentionally untested (istanbul provider)
