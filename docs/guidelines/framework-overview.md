...
- No nested barrel `index.ts` files; each module gets a sibling facade file (e.g. `src/error.ts` next to `src/error/`) that is the sole public entry point — internal barrel chains were removed to reduce package size and make imports explicit

## Naming and File Organization Patterns

- Files/folders use kebab-case (`not-found.ts`, `dependency-injection/`)
- Common suffixes are responsibility-based: `*-adapter.ts`, `*-server.ts`, `*-handler.ts`, `types.ts`
...
- Export through the module's facade file (e.g. `src/error.ts`) and keep docs aligned with the exported subpath.
