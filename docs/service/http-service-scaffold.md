...
export async function buildService() {
  const service = new Service({
    ...
    layers: {
      ...
      domain: {
        providers: [...catalystProviders, ...passportProviders, ...tenancyProviders],
      },
      infrastructure: {
        repositories: [...catalystRepositories, ...passportRepositories, ...tenancyRepositories],
        jobs: catalystJobs,
      },
    },
  });
...
- See [non-http-setup.md](./non-http-setup.md) for CLI/worker patterns
- Check [dependencies-reference.md](../third-party/dependencies-reference.md) for what's included
