...
These are exported via explicit subpaths for clarity and tree-shaking:
...
**Published tarball**: The npm package ships with a full `exports` map generated by [prepare-publish.ts](../../scripts/publish/prepare-publish.ts) at release time. This map is written to `publish/package.json` and rewrites local build paths to published paths:
...
```
@sentzunhat/zacatl/third-party/fastify
  → node_modules/@sentzunhat/zacatl/build-src-esm/third-party/fastify.js
```

No manual `tsconfig paths` config is needed because builds use real files, not virtual paths.

**Published package exports** (in `publish/package.json`) follow the three-condition format:
