# Migration Guide: v0.0.21 → v0.0.22

> ⚠️ **Historical Reference**: This guide documents the migration to v0.0.22's lazy loading pattern. Starting from v0.0.30+, BaseRepository initialization is **synchronous** and no longer uses lazy loading. See [current BaseRepository documentation](../reference/api/repository.md) for the modern approach.
...
### 🐛 Critical Bug Fix: ESM Compatibility
...
- ✅ Implemented lazy loading pattern with backward compatibility
- ✅ Repository constructors remain synchronous
- ✅ Adapters load on first async method call
...
### ⚠️ Accessing Model Before First Async Call

If you access the model synchronously after construction, you'll see a new error:

```typescript
const repo = new UserRepository();
repo.getMongooseModel(); // ❌ NEW ERROR in v0.0.22
...
repo.getMongooseModel(); // ✅ Now works
```
...
```typescript
constructor(config) {
  this.adapter = loadMongooseAdapter(config);  // ❌ Sync, used require()
}
```
...
```typescript
constructor(config) {
  this.config = config;  // ✅ Sync, stores config
}

private async ensureInitialized() {
  if (!this.adapter) {
    this.adapter = await loadMongooseAdapter(this.config);  // ✅ Async import()
  }
}
...
✅ **ESM Compatibility** - Works in Bun, Node.js ESM, Vite, Next.js
✅ **No Breaking Changes** - Existing code continues to work
✅ **Better Error Messages** - Clearer messages when ORMs are missing
✅ **Future-Proof** - Aligns with modern JavaScript/TypeScript ecosystem
...
- 🐛 Report issues: [GitHub Issues](https://github.com/sentzunhat/zacatl/issues)
