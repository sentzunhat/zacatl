# Migration Guide: v0.0.x to v0.1.0 (Future Release)

> **⚠️ IMPORTANT**: This guide describes planned changes for v0.1.0, which is **NOT YET RELEASED**.
> **Current stable version**: 0.0.33
> **Current API**: Uses `layers:` and `platforms:` (not `architecture:`)

## Overview

Version v0.1.0 will introduce enhanced multi-context architecture support. However, **the current codebase (v0.0.33) already uses the new structure** with `layers:` and `platforms:`. This guide describes the conceptual changes for when v0.1.0 is officially released.

---

## Current State (v0.0.33)

The framework **already supports** multi-context architecture:

```typescript
// CURRENT WORKING API (v0.0.33)
...
import {
  ...
  ApplicationRestHooks, // NEW (replaces ApplicationHookHandlers)
  ApplicationRestRoutes, // NEW (replaces ApplicationRouteHandlers)
...
// After
type MyHooks = ApplicationRestHooks;
type MyRoutes = ApplicationRestRoutes;
```
...
new Service({ layers: { ... }, platforms: { server: { ... } } });
new Service({ type: ServiceType.SERVER, layers: { ... }, platforms: { server: { ... } } });
```
...
### Issue 2: "ServiceType.SERVER requires 'platforms.server' configuration"
