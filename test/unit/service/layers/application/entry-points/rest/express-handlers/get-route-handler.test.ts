...
import { GetRouteHandler } from '../../../../../../../../src/service/layers/application/entry-points/rest/express/handlers/get-route-handler';
...
class TestGetRouteHandler extends GetRouteHandler<void, Record<string, string>, TestResponse> {
  async handler(_request: TestRequest): Promise<TestResponse> {
    return { id: 1, name: 'Test' };
  }
}

describe('Express GetRouteHandler', () => {
  it('should send the handler response when execute succeeds', async () => {
    const handler = new TestGetRouteHandler({ url: '/get-test', schema: {} });
