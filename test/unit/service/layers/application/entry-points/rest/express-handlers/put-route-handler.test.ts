...
import { PutRouteHandler } from '../../../../../../../../src/service/layers/application/entry-points/rest/express/handlers/put-route-handler';
...
class TestPutRouteHandler extends PutRouteHandler<void, Record<string, string>, TestResponse> {
  async handler(_request: TestRequest): Promise<TestResponse> {
    return { id: 1, name: 'Test' };
  }
}

describe('Express PutRouteHandler', () => {
  it('should send the handler response when execute succeeds', async () => {
    const handler = new TestPutRouteHandler({ url: '/put-test', schema: {} });
