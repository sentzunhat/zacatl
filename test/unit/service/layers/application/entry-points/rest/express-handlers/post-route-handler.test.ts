...
import { PostRouteHandler } from '../../../../../../../../src/service/layers/application/entry-points/rest/express/handlers/post-route-handler';
...
class TestPostRouteHandler extends PostRouteHandler<void, Record<string, string>, TestResponse> {
  async handler(_request: TestRequest): Promise<TestResponse> {
    return { id: 1, name: 'Test' };
  }
}

describe('Express PostRouteHandler', () => {
  it('should send the handler response when execute succeeds', async () => {
    const handler = new TestPostRouteHandler({ url: '/post-test', schema: {} });
