...
const shouldPrune = (rootDir: string, filePath: string, exportTargets: ReadonlySet<string>): boolean => {
  ...
  const buildName = path.basename(path.dirname(rootDir));
  const publishRel = normalizePath(path.join(buildName, rootName, rel));
  const sourceRel = normalizePath(
    path.join(rootName === 'esm' ? 'build-src-esm' : 'build-src-cjs', rel),
  );

  if (exportTargets.has(publishRel) || exportTargets.has(sourceRel)) return false;
