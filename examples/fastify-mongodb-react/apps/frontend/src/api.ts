...
export async function getGreetings(language?: string): Promise<Greeting[]> {
  ...
  const data = await request<ApiGreeting[]>(`${API_PREFIX}/greetings${query}`);
...
export async function getRandomGreeting(language: string): Promise<Greeting | null> {
  const data = await request<ApiGreeting | null>(
    `${API_PREFIX}/greetings/random/${encodeURIComponent(language)}`,
  );
...
export async function createGreeting(input: {
  ...
  const data = await request<ApiGreeting>(`${API_PREFIX}/greetings`, {
    method: 'POST',
    body: JSON.stringify({
...
export async function deleteGreeting(id: string): Promise<{ success: boolean }> {
  return request<{ success: boolean }>(`${API_PREFIX}/greetings/${id}`, {
    method: 'DELETE',
  });
}

export async function updateGreeting(id: string, input: { message: string }): Promise<Greeting> {
  const data = await request<ApiGreeting>(`${API_PREFIX}/greetings/${id}`, {
    method: 'PUT',
