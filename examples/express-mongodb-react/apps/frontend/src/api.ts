...
export async function getGreetings(language?: string): Promise<Greeting[]> {
  ...
  const data = await request<ApiGreeting[]>(`/api/greetings${query}`);
...
export async function createGreeting(input: {
  ...
  const data = await request<ApiGreeting>('/api/greetings', {
    method: 'POST',
    body: JSON.stringify({
...
export async function deleteGreeting(id: string): Promise<{ success: boolean }> {
  return request<{ success: boolean }>(`/api/greetings/${id}`, {
    method: 'DELETE',
  });
}
