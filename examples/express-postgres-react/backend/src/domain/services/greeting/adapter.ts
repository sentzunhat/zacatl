 ...
 *
 * This service can be used with ANY repository implementation (Sequelize, In-Memory, etc.)
...
import { GreetingRepositoryAdapter } from '../../../infrastructure/repositories/greeting/adapter';
...
export class GreetingServiceAdapter implements GreetingServicePort {
  ...
  async getGreeting(id: string): Promise<Greeting | null> {
    return this.greetingRepository.findById(id);
  }

  async getAllGreetings(language?: string): Promise<Greeting[]> {
    return this.greetingRepository.findAll(language ? { language } : undefined);
  }
