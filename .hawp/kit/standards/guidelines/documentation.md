...
// NOTE: pre_auth tokens must include tenancy claims; do not return cached session
// tokens directly if tenancy was added after session creation.
const token = await session.issue(sessionContent);
